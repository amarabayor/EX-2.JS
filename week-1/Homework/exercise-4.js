  ...
  */
  ...
  let restaurantFinderApplication = {
    ...
    findAvailableRestaurants: function (numberOfPeople) {
      return this.restaurants.filter((item) => { return item.totalSeats - item.numberOfCustomers >= numberOfPeople; }).map(item => item.name);
    },
    findRestaurantServingDish: function (dishName) {
      return this.restaurants.filter((item) => { return item.menu.find(i => i === dishName); }).map(item => item.name);
      
    },
    countNumberOfRestaurantsInArea: function (area) {
      let rests = this.restaurants.filter((item) => { return item.address.area === area; });
    return rests.length;
    },
  };
